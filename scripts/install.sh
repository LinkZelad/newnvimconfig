...
if [[ "${USE_SSH}" -eq "1" ]]; then
	if check_nvim_version "${REQUIRED_NVIM_VERSION}"; then
		execute "git" "clone" "-b" "main" "${CLONE_ATTR[@]}" "git@github.com:LinkZelad/newnvimconfig.git" "${DEST_DIR}"
		...
		execute "git" "clone" "-b" "0.8" "${CLONE_ATTR[@]}" "git@github.com:LinkZelad/newnvimconfig.git" "${DEST_DIR}"
	else
...
EOABORT
	...
	if check_nvim_version "${REQUIRED_NVIM_VERSION}"; then
		execute "git" "clone" "-b" "main" "${CLONE_ATTR[@]}" "https://github.com/LinkZelad/newnvimconfig.git" "${DEST_DIR}"
	elif check_nvim_version "${REQUIRED_NVIM_VERSION_LEGACY}"; then
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
		info "Automatically redirecting you to the latest compatible version..."
		execute "git" "clone" "-b" "0.8" "${CLONE_ATTR[@]}" "https://github.com/LinkZelad/newnvimconfig.git" "${DEST_DIR}"
	else
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION_LEGACY})."
		abort "$(
			cat <<EOABORT
